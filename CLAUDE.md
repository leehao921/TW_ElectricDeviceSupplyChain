...
## 分析框架 — 看科技股 / TW 供應鏈分析的核心提問

科技股的存在不是為了維持現狀，而是為了**打破現狀**。任何 thesis 都應先回答這五個問題：

1. **這個變化從哪裡開始？**
2. **誰的成本被降低？**
3. **誰的利潤被壓縮？**
4. **誰的價值鏈位置被重估？**
5. **誰只是短期受惠？誰才是真正拿走大部分增量利潤的人？**

### Product cycle 同步帶動的六層變化

一個產品週期啟動時，**同時**觸發：(1) 終端需求變化、(2) 供應鏈重新排序、(3) 零組件價值重估、(4) 產能瓶頸轉移、(5) 庫存拉貨節奏、(6) 客戶份額轉移 → 市場預期重新定價。寫任何 deep-dive 都要分這六層解構，不能只談需求。

### TW 公司分析必問（供應鏈角色）

TW 股大多是供應鏈玩家，**不直接面對終端消費者**。每家公司逐一回答：

- 在供應鏈裡的位置有沒有變好？
- 原本設計的供應鏈架構是否會改變？
- 跟著產業成長 vs 正在拿別人的 share？
- 受惠於產品週期 vs 剛好站在這一輪拉貨的某個節點？
- 結構性成長 vs 庫存週期幻覺？

### Long/Short pairing — 同一個變化同時創造贏家與輸家

科技股非常適合 long/short equity，因為**同一個變化必然伴隨對稱輸家**：

- 一家公司拿 share → 另一家掉 share
- 新規格被採用 → 舊規格供應商邊緣化
- 一個環節成瓶頸 → 另一個環節 commoditize

每個 long thesis 都要問：**對應的 short candidate 是誰？** Deep-dive 報告應在風險段落明示「誰會被擠壓」，不能只談受惠者。

### Channel check 資訊加權系統

聽到資訊**不要直接下結論** — 重點是判斷該給多少權重。每個訊號問四個問題：

1. **第幾手資訊？**
   - 供應商談自己 client：有參考價值
   - 供應商談 competitor：打折處理（聽來的，夾雜立場）
2. **這個人在產業裡看得到什麼？**
   - 業務看訂單、採購看價格、工程師看技術瓶頸、財務看數字、管理層看策略方向
   - **每人都可能講真話，但都只看到局部真相**；認真的分析要把局部真相拼成全局
3. **他為什麼要告訴我？**
   - 建立關係 / 影響市場預期 / 測風向 / 證明自己知道很多
   - 資訊不是免費的，**誘因結構決定資訊品質**
4. **他的 track record 如何？**
   - 資訊源本身需要被打分數，分數隨時間與互動調整

### Channel check 的三個層次

- **Expert network**：防守策略 — 避免「你不知道市場上其他人已經知道什麼」。Value add 已下降但仍要做。
- **內線親友**：藝術 — 知道什麼資訊有意義、去蕪存菁、維持健康長期關係。
- **最進階**：讓自己變成產業 key node，資訊主動流向你，甚至反過來影響公司決策（投資人是少數有機會對產業形成全局觀的人）。

### 寫 deep-dive 報告時的具體要求

每份產業 deep-dive / 個股分析應：

- 明示「變化從哪裡開始」（catalyst origin），不能只寫「最近上漲」
- 列出贏家與輸家**對偶名單**（誰受惠 → 對偶誰被擠壓）
- 區分**結構性 vs 庫存週期**驅動
- Channel check 資料引用必標 confidence marker（IR/法說會 vs 媒體 vs 推測），並寫明「為什麼這個 source 該被信」
- 對任何 share gain 主張，**列出對偶 short candidate**（即使本報告只 long 一方）
