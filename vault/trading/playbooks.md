...
## 核心交易邏輯總綱 (2026-08-24 本人定調)

**短線引擎 — 定價錯誤:**
買入條件 = 錯價證據 (基本面/籌碼/價格三者背離),非動能。左側需賣壓竭盡確認 (外資流向轉正) 或觸發條件 (華航三觸發模式)。隔日沖回測已證: 追價=負期望。

**中長期主題 — 地緣政治經濟 × 先進半導體/電子鏈:**
判斷單位是「地緣格局如何重塑供應鏈位置」(美中管制/美伊油價/供應鏈重組),不是單一公司財報。CSP ASIC、CoWoS 瓶頸、AI 空運鏈等研究皆屬此層。

**資金循環引擎:**
定價錯誤買入 → 高點 (乖離 98pct+/P/B RED/殖利率天花板) 收穫 → 再投入下一個錯價標的。
永遠從「定價過高」往「定價過低」搬錢。實例: 2408 tp2 收割→左側三段;2603 停利→華航/仁寶。

**存股底盤 + 黑天鵝 playbook:**
- 底盤: 大型權值股/跨產業高殖利率/半導體 ETF,存股不動
- 黑天鵝 (regime broken / 4月7月式崩跌): **質押權值股重複買進** — 方向是加碼不是減碼
- 股利收益在低點再投入,稀釋成本為目標函數
- 槓桿紀律: 融資=短線例外 (事件 gate+時間停利),質押=黑天鵝專用

**執行對照:** 短線倉 (仁寶融資/左側三段) ↔ 循環倉 (2603/2408 階梯) ↔ 存股倉 (00891/00999A/權值) 三層帳分開管理,訊號不互串。

## 無限轉倉回檔應對 — 兩階段避險排名 (2026-08-29 本人分享)

適用: 期貨多方滾倉部位 (TX/TMF 無限轉倉) 遇不如預期回檔時的選擇權調整。
核心變數 = **IV 與 skew 的當下位置** — 兩階段排名反轉的原因是保護成本結構改變。

### ① 初跌/剛轉弱 (IV 尚未爆高 → 買保護還便宜)
排名: **BP淺價外 > BP價平 > Buy Put Spread > BP深價外 > SC價外 > SC價平 > SC價內 > SC深價內**
- BP 淺價外: 成本/保護最均衡, IV 未爆是提早避險的窗口
- SC 系列一律墊底: 收小權利金換不到下跌保護, 且犧牲反彈

### ② 已跌一大段/恐慌中 (IV 爆高 + put skew 拉開 → 買保護變貴)
排名: **Buy Put Spread > BP價平 > BP淺價外 > BP深價外 > SC價外 > SC價平 > SC價內 > SC深價內**
- Spread 登頂原因: short leg 回收爆貴的 IV/skew, 抵銷買方成本
- SC 價內/深價內最忌: 低檔鎖死反彈 — 與黑天鵝「跌深要加碼不是減碼」哲學同構

### 階段判別 — 用系統日更數據量化 (vix_daily, margin-vix 18:10 推送)
| 訊號 | ① 初跌 | ② 恐慌 |
|---|---|---|
| VIX / 週選 vix_w | 未爆 (~<28) | 爆高 (>32, 7/29 週選曾 38) |
| 週/月 wm_spread | ≤ +2 (未倒掛) | > +2 倒掛 🚨 (7/29=+8.2, 8/5=+10.4) |
| VRP30 | 中性/薄 (<8) — 保險便宜 | 肥厚 (>8) — 保險貴, spread 優先 |
| 25Δ skew | 正常 | 深負 (價外 put 相對爆貴) |
| GEX zone (dashboard) | 磁吸→放大切換初期 | 放大區深處 (負 gamma 加速) |

**操作含義**: wm_spread 倒掛 >+2 當日 = ①→② 的切換鐘 — 之後別再追買裸 put, 改 spread。
sf-pairs 週報選擇權環境段已內建同邏輯 (VRP<0 put 優先 / VRP>8 期貨蓋板)。
