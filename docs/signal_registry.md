...
## 盤中微結構 — TXF × GEX 水位（2026-09-01 物理檢定，analysis/gex_physics_20260901.md）

| 效應 | 檢定結果 | 分級 | agent 用法 |
|---|---|---|---|
| ZG regime RV 切換 | S<ZG 日 RV 較大 **28/28 日** (p=0.000)，比值中位 1.34x（spec 1.5x 未達）| **方向 VALIDATED / 力度弱** | EXPANSION 態 → 縮部位放寬停損；非進出場訊號 |
| PW 觸牆反彈 | t15 反轉勝率 **62%** (n=214, p=0.001)，中位 +25 點 | **VALIDATED**（唯一可交易級）| 均值回歸腿：接近 PW 由上緣未破 → 反彈期望 |
| CW 觸牆壓回 | t5 57% (p=0.055)、t15 53% (ns) | WEAK | 僅作壓力參考，不宜單獨 fade |
| 牆面貫穿追擊 | CW mean **-38** vs median +3；PW mean **-145**（跌破反被拉回）；勝率 ~50% | **不成立**（開盤靜態水位下負期望）| 勿用突破追擊；跌破 PW 反而偏回歸 |

> 口徑：開盤 09:00-09:30 gamma×D-1 OI 靜態水位、日盤 1-min、74 日（ZG 28 日配對）。盤中動態水位（gex-regime 5min feed）效應未檢定，屬後續題。
